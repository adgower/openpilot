...
1. `package.py` creates complete-ancestry parent and submodule bundles with advertised exact pins. All Git bundles verify against an empty repository. Every frozen LFS payload is separately hash-verified and included; checkout disables network smudging and restores only matching payloads. Package SHA256SUMS covers bundles, LFS data, manifest and scripts; no upstream fetch/push/merge is needed to materialize them. Package creation uses source Git objects, never a copy of a dirty working tree.
